...
const Template = ({ ...args }) => (
  <>
    <DesignSystemGlobals />
    <Header $format="Headline/Medium/Bold">Lorem ipsum dolor sit amet, consectetur adipiscing elit.</Header>
...
export default {
  argTypes: {
    ...
    $format: {
      ...
      options: getAllResponsiveOptions(
        Object.fromEntries(
          getAllFontTokens()
            .filter(token => Boolean(token.match(/Body|Label/)))
            .map(token => {
              const [, ...restToken] = OLDfontTokenToTuple(token);
  ...
  component: Text,
  title: 'OLD Design System/Components/Text',
};
